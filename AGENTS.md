...
related: [claude-instructions, org-governance-adoption, engineering-standards, contributing]
---

# AGENTS.md

**This file is the source of truth for agent instructions in this repository.**
`CLAUDE.md` imports it and adds nothing of its own. Tool-neutral by design: any
...
- Naming: `<prefix>/<id>-<short-description>`, prefixes `feat`, `fix`,
  ...
  matrix live in `git-governance`'s `agents/git-governance-advisor.md`.
...
- **Every merge goes through a pull request, including into `develop`.** The
  ruleset requires one, at `required_approving_review_count: 0` — a solo
  maintainer cannot approve their own PR, so any non-zero count would deadlock
  the branch. Zero approvals is a deliberate choice, not a missing setting.
...
It would be better if GitHub refused a merge into `staging` or `main` performed
by an agent. It cannot: the agent acts through the owner's token, and GitHub's
ACLs have no way to tell the two apart when they share one identity. The obvious
substitute — requiring one approving review — makes it *worse*, not better,
because GitHub forbids approving your own pull request, so a solo maintainer
would be locked out of their own promotion branches entirely.

The only genuinely server-side version of this control requires giving the agent
a **separate identity** (a GitHub App installation token or a machine user),
then requiring a review on `staging`/`main` with the owner as a bypass actor.
That works, and it is the standard least-privilege answer, but it changes how
Claude Code authenticates and is not in force today.

So the gate is behavioral, and it is written down in three places on purpose:
here, in the advisor subagent, and in each promotion command. Do not "fix" it by
raising the approval count.

### Merge methods

Merge commits are the default everywhere; squash is a narrow exception and
rebase-merge is off entirely. This is enforced by ruleset, per branch, not left
...
promoted commits into a new one, so `staging` no longer shares history with
`develop` and the *next* promotion re-conflicts on changes that were already
merged. That failure mode has been hit in this workspace before; merge-only on
the promotion branches is what prevents it structurally.

Squash stays available into `develop` for work branches whose intermediate
commits carry no value. Rebase-merge is disabled at the repository level because
it offers nothing this model needs and rewrites committer metadata on the way in.

### Branch lifecycle

- `delete_branch_on_merge` is **on**: work branches are deleted from the remote
  automatically when their PR merges, so cleanup is a repository setting rather
  than a step repeated by hand on every merge.
- `develop`, `staging`, and `main` are protected against deletion by the
  `deletion` rule in their ruleset. Auto-delete cannot reach them: GitHub exempts
  protected branches from it, and the rule blocks the deletion regardless. This
  matters because `develop` *is* the head branch of a `develop -> staging` pull
...
Each governed fact — the branch-prefix taxonomy, the permission matrix, the
frontmatter exceptions register, an enforcement claim like the CI guard's clause
count — has exactly **one** authoritative file. Every other file links to it
(`"see X"`) instead of restating it in its own prose:

- Taxonomy and the permission matrix → owned by `git-governance`'s
...
- The frontmatter exceptions register → owned by `.docgov.config.js`, with
  [docs/org-governance-adoption.md](docs/org-governance-adoption.md) restating
  it under a `facts` pin.

This repo uses `docs-governance` (see "Companion plugins" above) to catch
restatement drift mechanically where it can. If an audit — human, `docgov`, or
an LLM auditor — finds the same fact stated in 2+ files, the fix is to add or
extend a `facts` entry in `.docgov.config.js`, not just correct the wording in
place: correcting the wording alone leaves nothing keeping the next edit from
re-breaking it.
